...
const Work = () => {
  return (
    <section id="work" className="relative bg-black text-white py-16 md:py-24">
      <div className="absolute inset-0 bg-[radial-gradient(700px_400px_at_50%_0%,rgba(255,200,120,0.08),transparent_60%)]" />
      <div className="relative container mx-auto px-6">
        <h2 className="text-2xl md:text-3xl font-light tracking-tight text-white/90">Работы</h2>
        <p className="mt-3 text-white/70 max-w-2xl">Атмосферные кадры из живых проектов: тёплые заливки, чистые белые лучи и эмоции гостей.</p>
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {images.map((src, idx) => (
            <div key={idx} className="group relative overflow-hidden rounded-2xl bg-white/5 border border-white/10">
              <img src={src} alt="Кадр из проекта" className="h-64 w-full object-cover transition-transform duration-700 group-hover:scale-[1.03]" />
